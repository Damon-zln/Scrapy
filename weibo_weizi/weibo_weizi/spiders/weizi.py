...
class WeiziSpider(Spider):
    ...
    # 粉丝列表API
    fans_url = 'https://m.weibo.cn/api/container/getIndex?containerid=231051_-_fans_-_{uid}&since_id={since_id}'
    # 薇姿ID
    start_users = ['1976287937']
    ...
    def parse(self, response):
        ...
        if result.get('data').get('userInfo'):
            ...
            # 粉丝
            yield Request(self.fans_url.format(uid=uid, since_id=1), callback=self.parse_fans, meta={'since_id': 1, 'uid': uid})
            # 微博
    ...
    def parse_fans(self, response):
        ...
        if result.get('ok') and result.get('data').get('cards') and len(result.get('data').get('cards')) and result.get('data').get('cards')[-1].get('card_group'):
            ...
            yield Request(self.fans_url.format(uid=uid, since_id=since_id), callback=self.parse_fans, meta={'since_id': since_id, 'uid': uid})
    ...
    def parse_weibo(self, response):
        ...
        if result.get('ok') and result.get('data').get('cards'):
            ...
            for weibo in weibos:
                ...
                if mblog:
                    ...
                    field_map = {
                        ...
                        'source': 'source', 'text': 'text', 'created_at': 'created_at'
                    }
                    for field, attr in field_map.items():
            ...
            uid = response.meta.get('uid')
            page = response.meta.get('page') + 1
            yield Request(self.weibo_url.format(uid=uid, page=page), callback=self.parse_weibo, meta={'page': page, 'uid': uid})
